        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div class="bg-white rounded-3xl shadow-lg overflow-hidden border border-gray-100">
                    @if(isset($casa))
                        ...
                        <div class="p-6">
                            ...
                            <p class="text-gray-500 mb-4"><i class="fas fa-map-marker-alt text-red-500 mr-2"></i>{{ $casa->ubicacion }}</p>
                            <div class="border-t border-dashed border-gray-200 my-4"></div>
                            <p class="text-gray-600 text-sm">Deja tu opinión y valoración sobre esta casa. Los testimonios serán revisados antes de publicarse.</p>
                        </div>
                    @else
                        <div class="p-6">
                            <h3 class="text-xl font-bold text-gray-900 mb-2">Comparte tu experiencia</h3>
                            <p class="text-gray-600 text-sm">Selecciona la casa a la que corresponde tu testimonio en el formulario.</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div>
                <div class="bg-white rounded-3xl shadow-xl p-8 border border-gray-100 relative">
                    <form action="{{action([App\Http\Controllers\TestimonioController::class, 'store']) }}" method="POST">
                        @csrf @method('POST')
                        @if(isset($casa))
                        ...
                        @else
                            <div class="mb-5">
                                ...
                                <select name="casa_id" required class="w-full px-4 py-3 rounded-xl border border-gray-300 bg-gray-50">
                                    <option value="">Selecciona una casa</option>
                                    <option value="">Comentario general sobre la plataforma</option>
                                    @foreach($casas ?? [] as $c)
                                        <option value="{{ $c->id }}" {{ old('casa_id') == $c->id ? 'selected' : '' }}>{{ $c->nombre_casa }} — {{ $c->ubicacion }}</option>
                                    @endforeach
                                </select>
                                @error('casa_id') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                            </div>
                        @endif

                        <div class="mb-6">
                            <h3 class="text-xl font-bold text-gray-900">Tu testimonio</h3>
                            <p class="text-sm text-gray-400">Sé honesto y concreto. El testimonio se revisará antes de publicarse.</p>
                        </div>

                        <div class="space-y-5">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Contenido</label>
                                <textarea name="contenido" required rows="6" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition bg-gray-50">{{ old('contenido') }}</textarea>
                            ...
                            <div>
                                ...
                                <input type="number" name="valoracion" required min="1" max="5" value="{{ old('valoracion', 5) }}" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition bg-gray-50">
                                @error('valoracion') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                            </div>
                        </div>
